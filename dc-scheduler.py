...
COMMUNITY_CRAWLER_NLP_IMAGE = "usa6463/community-crawler-nlp:v0.3.1"
...
def dc_scrapping():
    ...
    monthly_brand_from_static_name_pool = KubernetesPodOperator(
        ...
        cmds=["pipenv", "run", "run_monthly_brand_from_static_name_pool"],
        retries=1
    )

    extract_type_from_static_name_pool = KubernetesPodOperator(
        name="extract_type_from_static_name_pool",  # pod name
        namespace="airflow",
        env_vars=[
            V1EnvVar(name="ES_URL", value=ES_URL),
            V1EnvVar(name="PGSQL_URL",
        ...
        image=("%s" % COMMUNITY_CRAWLER_NLP_IMAGE),
        task_id="extract_type_from_static_name_pool",
        container_resources=k8s_models.V1ResourceRequirements(
            limits={"memory": "2G", "cpu": "2000m"},
        ),
        cmds=["pipenv", "run", "run_extract_type_from_static_name_pool"],
        retries=1
    )

    monthly_type_from_static_name_pool = KubernetesPodOperator(
        name="monthly_type_from_static_name_pool",  # pod name
        namespace="airflow",
        env_vars=[
            V1EnvVar(name="PGSQL_URL",
                     value=PGSQL_URL),
        ...
        image=("%s" % COMMUNITY_CRAWLER_NLP_IMAGE),
        task_id="monthly_type_from_static_name_pool",
        container_resources=k8s_models.V1ResourceRequirements(
            limits={"memory": "2G", "cpu": "2000m"},
        ),
        cmds=["pipenv", "run", "run_monthly_type_from_static_name_pool"],
        retries=1
    )
    ...
    man_fashion_gall >> extract_type_from_static_name_pool >> monthly_type_from_static_name_pool
